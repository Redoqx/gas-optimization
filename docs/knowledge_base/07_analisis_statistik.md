...
|---|---|---|---|---|
| Wilcoxon Signed-Rank | W = 15.0 | **0.031** | ✅ Ya | H1 Diterima |
| Chi-square (domain) | χ² = 8.568 | 0.073 | ❌ Tidak | H0 Diterima |
| Kruskal-Wallis (complexity) | H = 0.631 | 0.427 | ❌ Tidak | H0 Diterima |
| Spearman (LOC vs findings) | ρ = -0.261 | 0.079 | ❌ Tidak | H0 Diterima |
...
|---|---|---|
| Apakah optimasi gas signifikan secara statistik? | **Ya** (Wilcoxon p=0.031) | Framework valid — pola yang dideteksi memang boros gas |
| Apakah domain mempengaruhi keberadaan anti-pattern? | **Tidak** (p=0.073) | Framework berguna lintas domain |
| Apakah kontrak kompleks lebih banyak anti-pattern? | **Tidak** (p=0.427) | Ukuran kontrak bukan prediktor jumlah temuan |
| Apakah kontrak lebih panjang lebih banyak temuan? | **Tidak** (ρ=-0.26, p=0.079) | LOC tidak bisa memprediksi jumlah findings |

**Kesimpulan statistik**: Satu-satunya temuan yang signifikan secara statistik adalah bahwa **optimasi gas yang disarankan framework terbukti efektif** (Wilcoxon). Distribusi dan jumlah findings tidak dipengaruhi oleh karakteristik kontrak (domain, complexity, LOC).

---

## Uji Tambahan: McNemar (Gagal Dilakukan)

Uji McNemar direncanakan untuk membandingkan detektor kita vs Slither secara biner per kontrak. Namun karena Slither tidak berhasil menganalisis satupun dari 10 kontrak sampel (ketidakkompatibilan versi solc), tabel kontingensi menjadi:
- a = 0 (both found)
- b = 8 (only ours)
- c = 0 (only Slither)
- d = 2 (neither)

Karena b+c = 8 (hanya satu arah), McNemar tidak dapat mengukur "disagreement" yang bermakna. Ini dicatat sebagai **keterbatasan metodologis** akibat inkompatibilitas Slither dengan kontrak Solidity 0.4.x.
